...
class Evento(Base):
    ...
    listas_convidados = relationship("ListaConvidados", back_populates="evento")
...
# ==================== MODELOS KDS PRINCIPAIS ====================
# Definindo primeiro as tabelas principais que serão referenciadas

class StatusKds(enum.Enum):
    PENDENTE = "pendente"
...
class EstacaoKds(Base):
    """Estações do Kitchen Display System"""
    __tablename__ = "estacoes_kds"
    
    id = Column(Integer, primary_key=True, index=True)
    nome = Column(String(100), nullable=False)
    descricao = Column(Text)
    tipo = Column(Enum(TipoEstacao), default=TipoEstacao.PRINCIPAL)
    
    # Configurações de exibição
    cor_tema = Column(String(7), default="#1E40AF")  # Cor hexadecimal
    layout_colunas = Column(Integer, default=3)
    tempo_alerta_minutos = Column(Integer, default=15)
    
    # Status e configurações
    ativa = Column(Boolean, default=True)
    auto_atualizacao = Column(Boolean, default=True)
    ...
    itens_kds = relationship("ItemKds", back_populates="estacao")

class PedidoKds(Base):
    """Pedidos do sistema KDS"""
    ...
    status = Column(Enum(StatusKds), default=StatusKds.PENDENTE)
    prioridade = Column(Integer, default=1)  # 1=normal, 2=alta, 3=urgente
    ...
    estimativa_entrega = Column(DateTime(timezone=True))
    finalizado_em = Column(DateTime(timezone=True))
    entregue_em = Column(DateTime(timezone=True))
    
    # Observações gerais
    observacoes = Column(Text)
    origem = Column(String(20), default="balcao")  # balcao, delivery, mesa
    
    # Relacionamentos
    itens = relationship("ItemKds", back_populates="pedido")

class ItemKds(Base):
    """Itens/pedidos exibidos no KDS"""
    __tablename__ = "itens_kds"
    ...
    # Status e timing específico do item
    status = Column(Enum(StatusKds), default=StatusKds.PENDENTE)
    prioridade = Column(Integer, default=1)  # 1=normal, 2=alta, 3=urgente
    tempo_estimado_minutos = Column(Integer, default=10)
    ...
    estacao = relationship("EstacaoKds", back_populates="itens_kds")
...
class Ticket(Base):
    ...
    id = Column(Integer, primary_key=True, index=True)
    tipo_ticket_id = Column(Integer, ForeignKey("tipos_ticket.id"))
    lote_id = Column(Integer, ForeignKey("lotes_tickets.id"))
    cliente_id = Column(Integer, ForeignKey("clientes_eventos.id"))
    codigo = Column(String(50), unique=True, nullable=False)
    qr_code = Column(Text)
...
# ==================== MODELOS LISTA DE CONVIDADOS ====================

class ListaConvidados(Base):
    """Lista de convidados para eventos"""
    __tablename__ = "listas_convidados"
    ...
    # Informações básicas
    nome = Column(String(200), nullable=False)
    tipo = Column(String(50), nullable=False)  # VIP, FREE, PAGANTE, PROMOTER
    descricao = Column(Text)
    
    # Configurações
    codigo_convite = Column(String(50), unique=True, nullable=False)
    link_convite = Column(String(500))
    quantidade_maxima = Column(Integer)
    permite_acompanhantes = Column(Boolean, default=False)
    quantidade_acompanhantes = Column(Integer, default=0)
    
    # Preços e valores
    preco_masculino = Column(Float, default=0.0)
    preco_feminino = Column(Float, default=0.0)
    preco_unissex = Column(Float, default=0.0)
    
    # Datas e horários
    data_abertura = Column(DateTime(timezone=True))
    data_fechamento = Column(DateTime(timezone=True))
    horario_limite_entrada = Column(Time)
    
    # Configurações de exibição
    exibir_no_app = Column(Boolean, default=True)
    lista_fixa = Column(Boolean, default=False)
    ...
    ativa = Column(Boolean, default=True)
    total_convidados = Column(Integer, default=0)
    total_confirmados = Column(Integer, default=0)
    ...
    # Relacionamentos
    evento = relationship("Evento", back_populates="listas_convidados")
    promoter = relationship("Usuario", foreign_keys=[promoter_id])
    criador = relationship("Usuario", foreign_keys=[criado_por])
    convidados = relationship("ConvidadoLista", back_populates="lista", cascade="all, delete-orphan")
    categoria_cliente = relationship("CategoriaCliente")

class ConvidadoLista(Base):
    """Convidados em uma lista"""
    __tablename__ = "convidados_lista"
    
    id = Column(Integer, primary_key=True, index=True)
    lista_id = Column(Integer, ForeignKey("listas_convidados.id"), nullable=False)
    ...
    # Acompanhantes
    quantidade_acompanhantes = Column(Integer, default=0)
    nomes_acompanhantes = Column(Text)  # JSON com nomes
    
    # Status
    confirmado = Column(Boolean, default=False)
    data_confirmacao = Column(DateTime(timezone=True))
    checkin_realizado = Column(Boolean, default=False)
    data_checkin = Column(DateTime(timezone=True))
    ...
    # Origem
    origem_adicao = Column(String(50))  # app, site, promoter, admin
    adicionado_por = Column(Integer, ForeignKey("usuarios.id"))
    ...
    # Relacionamentos
    lista = relationship("ListaConvidados", back_populates="convidados")
    usuario_adicao = relationship("Usuario")
    
    # Constraint única para evitar duplicação
    __table_args__ = (
        UniqueConstraint('lista_id', 'cpf', name='_lista_cpf_uc'),
    )
