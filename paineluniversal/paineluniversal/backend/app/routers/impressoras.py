...
@router.get("/status")
async def obter_status_geral(
    db: Session = Depends(get_db),
    current_user: Usuario = Depends(obter_usuario_atual)
):
    """Obter status geral de todas as impressoras"""
    impressoras = db.query(Impressora).all()
    
    total = len(impressoras)
    online = sum(1 for imp in impressoras if imp.status == StatusImpressoraEnum.ONLINE)
    ...
    return {
        ...
        "online": online,
        "offline": offline,
        "percentual_online": round((online / total * 100) if total > 0 else 0, 1),
