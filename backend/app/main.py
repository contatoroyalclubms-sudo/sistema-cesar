...
from .routers import auth, eventos, usuarios, empresas, listas, transacoes, checkins, dashboard, relatorios, whatsapp, cupons, n8n, pdv, gamificacao, produtos, formas_pagamento, meep, financeiro, bi, split  # import_export
...
def run_startup_migrations():
    """Executa migracoes automaticas no startup se for deployment Railway"""
    try:
        deploy_monitor.log_startup_info()
        
        # Verificar se e ambiente Railway
        is_railway = os.getenv("RAILWAY_ENVIRONMENT") is not None
        database_url = os.getenv("DATABASE_URL")
        
        # TEMPORARIAMENTE DESABILITADO - Schema ja migrado
        logger.info("Migracoes automaticas temporariamente desabilitadas")
        return
        
        if is_railway and database_url:
            logger.info("Ambiente Railway detectado - Executando migracoes automaticas...")
            ...
            if migration_success:
                logger.info("Migracoes automaticas concluidas com sucesso")
            else:
...
async def startup_event():
    ...
    if migration_success:
        logger.info("Migracao automatica concluida com sucesso")
    else:
...
app.include_router(financeiro.router, prefix="/api/financeiro", tags=["Financeiro"])
app.include_router(bi.router, tags=["Business Intelligence"])
app.include_router(split.router, tags=["Split Pagamentos"])
